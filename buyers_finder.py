...
async def find_buyers(mint: str, target_date: datetime) -> BuyersResult:
    """
    Find wallets that received (bought) the given token on the target_date (UTC).
    Uses getSignaturesForAddress on the mint + parse token balance changes.
    """
    ...
    buyers_map: Dict[str, BuyerInfo] = {}  # wallet -> best (largest) buy
    seen_wallets: Set[str] = set()

    async with SolanaRPC() as rpc:
        # 1. Collect signatures that fall inside the day
        ...
        while total_fetched < max_sigs:
            ...
            for sig_info in batch:
                ...
                if block_time >= end_ts:
                    # still in the future relative to our day ’ keep going older
                    continue
                if block_time < start_ts:
                    # older than our day ’ we can stop
                    stop = True
                    break
        ...
        # 2. Parse transactions (limit to avoid long waits)
        ...
        for i, sig_info in enumerate(to_process):
            ...
            # Parse pre/post token balances for this mint
            pre_balances = {
            ...
            # Find owners who increased their balance
            ...
            for owner in all_owners:
                ...
                if delta > 0.000001:  # received tokens (buy or transfer in)
                    # Prefer larger buys
